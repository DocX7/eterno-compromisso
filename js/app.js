const APP_VERSION = 'V24_ESSENCIAL_FINAL';
...
const STORAGE_KEY = 'ec_v24_essencial_state';
const LEGACY_STORAGE_KEYS = ['ec_v23_essencial_state','ec_v22_essencial_state'];
...
function autoBackup(){
  try{
    const k='ec_v24_auto_backups'; const today=todayKey();
...
function render(){ renderShell(); renderHoje(); renderCouple(); renderChild(); renderReading(); renderSettings(); }
function renderShell(){
  $$('.section').forEach(x=>x.classList.toggle('active',x.id==='sec-'+(S.activeTab||'hoje')));
  $$('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.tab===(S.activeTab||'hoje')));
...
function renderHoje(){
  ...
  el.innerHTML=`
    <div class="hero">
      ...
      <h2>O que vamos fazer hoje com Deus?</h2>
      <p>Três passos simples: Palavra, devocional do casal e uma verdade bíblica para Arthur.</p>
      <div class="syncLine"><span class="dot ${syncDotClass()}"></span><span>${safeHTML(syncLabel())}</span></div>
    </div>

    <div class="actionGrid">
      <button type="button" class="action" onclick="switchTab('casal')"><span class="ico">♡</span><span><strong>Devocional do casal</strong><span>Reflexão teológica, exame do coração e prática de hoje.</span></span></button>
      <button type="button" class="action" onclick="switchTab('crianca')"><span class="ico">✦</span><span><strong>Devocional do Arthur</strong><span>Tema de hoje: ${safeHTML(childThemeForToday().name)}.</span></span></button>
      <button type="button" class="action" onclick="switchTab('leitura')"><span class="ico">📖</span><span><strong>Leitura bíblica</strong><span>Dia ${p.day}: ${safeHTML(p.reading)}</span></span></button>
    </div>

    <div class="grid two">
      <div class="card">
        <div class="row between"><h3>Momento devocional</h3><span class="pill">${sc}%</span></div>
  ...
  $$('.check input').forEach(i=>i.onchange=()=>markCheck(i.dataset.id,i.checked));
}
...
function saveWeeklyReview(){
  ...
  save(); toast('Revisão semanal salva.');
}
...
async function generateDevotion(type, force=false){
  const k=todayKey();
  if(!force && S.devotions[type]?.[k]?.text){ toast('O devocional de hoje já está salvo.'); return; }
  ...
  try{
    ...
    save(); toast('Devocional salvo para hoje.');
  }catch(e){
    console.warn(e);
    ...
    save(); toast('IA indisponível. Salvei uma versão local.');
  }
}
...
function renderCouple(){
  ...
  el.innerHTML=`
    <div class="hero"><p class="kicker">Devocional diário do casal</p><h2>Teologia para dentro da aliança.</h2><p>Uma reflexão para ler com reverência, conversar com honestidade e praticar o evangelho dentro de casa.</p></div>
    <div class="card">
...
function renderChild(){
  ...
  el.innerHTML=`
    <div class="hero"><p class="kicker">Devocional infantil</p><h2>Para Arthur, 3 anos.</h2><p>Curto, repetível, carinhoso e adequado para ensinar uma verdade bíblica simples.</p><span class="themePill">Tema: ${safeHTML(t.name)}</span><span class="themePill">Frase: ${safeHTML(t.phrase)}</span></div>
    <div class="card">
...
function openReadMode(type){
  ...
  $('#readerTitle').textContent = type==='couple' ? 'Modo leitura sem distração' : 'Momento com Arthur';
  $('#readerBody').textContent = text;
  const btn=$('#readerComplete'); btn.onclick=()=>{ completeDevotion(type); closeReadMode(); };
...
function markRead(day,val){
  ...
  save(); toast(val?'Dia marcado como lido.':'Dia desmarcado.');
}
function setCurrentDay(day){ S.reading.currentDay=clamp(day,1,365); save(); toast('Dia atual ajustado.'); }
function resetReading(){
  if(confirm('Resetar todo o progresso de leitura? Vocês poderão marcar manualmente o que já leram.')){
    S.reading={...emptyState().reading,startDate:todayKey(), currentDay:1, readDays:{}, notes:{}, resetAt:Date.now()}; save(); toast('Plano resetado.');
  }
}
function markUntil(day){
  ...
  S.reading.currentDay=clamp(day+1,1,365); save(); toast('Leituras marcadas.');
}
...
function setStartToday(){ if(confirm('Definir hoje como o dia 1 do plano? Isso não apaga leituras marcadas.')){ S.reading.startDate=todayKey(); save(); toast('Calendário ajustado.'); } }
...
function renderReading(){
  ...
  el.innerHTML=`
    <div class="hero"><p class="kicker">Plano cronológico 365 dias</p><h2>Leitura bíblica anual, leve no celular.</h2><p>A tela mostra o essencial. A lista completa só aparece quando vocês pedirem.</p></div>
    <div class="grid two">
      <div class="card">
        <h3>Progresso</h3>
    ...
    <div class="card"><h3>Próximos 7 dias</h3><div class="compactList">${next||'<div class="empty">Plano concluído.</div>'}</div></div>
    <div class="card"><h3>Dias anteriores próximos</h3><div class="compactList">${previous||'<div class="empty">Vocês estão no início do plano.</div>'}</div></div>
...
function saveNote(id,val){ S.reading.notes[id]=val; save(); toast('Reflexão salva.'); }

function exportBackup(){ const blob=new Blob([JSON.stringify(S,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='backup-eterno-compromisso-v24-'+todayKey()+'.json'; a.click(); URL.revokeObjectURL(a.href); }
function importBackup(file){ if(!file)return; const r=new FileReader(); r.onload=()=>{ try{ const imported=JSON.parse(r.result); S={...emptyState(),...imported,version:APP_VERSION}; normalizeState(); save(); toast('Backup restaurado.'); }catch(e){ toast('Arquivo inválido.'); } }; r.readAsText(file); }
function restoreAutoBackup(index=0){
  try{
    const backups=JSON.parse(localStorage.getItem('ec_v24_auto_backups')||'[]');
...
function renderSettings(){
  ...
  const hist=(S.history.completedDevotions||[]).slice(0,6).map(x=>`<div class="historyItem">${x.date} — ${x.type==='couple'?'Devocional do casal':'Devocional do Arthur'}<br><span>${safeHTML(x.reading||'')}</span></div>`).join('') || '<p class="sub">Nenhum devocional concluído ainda.</p>';
  let backups=[]; try{ backups=JSON.parse(localStorage.getItem('ec_v24_auto_backups')||'[]'); }catch(e){}
  const backupList=backups.slice(0,4).map((b,i)=>`<div class="historyItem"><strong>${b.date}</strong><br><span>Backup automático local</span><br><button type="button" class="btn mini secondary" onclick="restoreAutoBackup(${i})">Restaurar</button></div>`).join('') || '<p class="sub">O primeiro backup automático será criado após salvar dados hoje.</p>';
  el.innerHTML=`
    <div class="hero"><p class="kicker">Ajustes</p><h2>Essencial, leve e confiável.</h2><p>Conta, backup, sincronização, histórico simples e manutenção do aplicativo.</p><div class="syncLine"><span class="dot ${syncDotClass()}"></span><span>${safeHTML(syncLabel())}</span></div></div>
    <div class="grid two">
      ...
      <div class="card"><h3>Diagnóstico</h3><div class="stats"><div class="stat"><b>24</b><span>Versão</span></div><div class="stat"><b>${doneCount()}</b><span>Dias lidos</span></div><div class="stat"><b>${cloudReady?'Nuvem':'Local'}</b><span>Status</span></div></div><button type="button" class="btn secondary full" style="margin-top:10px" onclick="clearCaches()">Limpar cache</button></div>
      <div class="card"><h3>Histórico simples</h3><div class="history">${hist}</div></div>
    </div>`;
}
async function clearCaches(){ if('caches' in window){ const ks=await caches.keys(); await Promise.all(ks.map(k=>caches.delete(k))); } toast('Cache limpo. Reabra o app.'); }
...
(async function main(){ localLoad(); await loadDataFiles(); installSW(); initFirebase(); render(); })();
